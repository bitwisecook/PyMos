...
def build_mosaic(input_path,
    ...
    if os.path.exists(colormap_file) and not new_colormap:
        ...
        if not USING_RTREE:
            pickle.dump(colormap, open(colormap_file, 'wb'))
    ...
    for s_x in range(0, output_width, thumb_size):
        for s_y in range(0, output_height, thumb_size):
